...
const predictYield = async (inputData) => {
  ...
  for (const field of requiredFields) {
    ...
    if (['Year', 'average_rain_fall_mm_per_year', 'pesticides_tonnes', 'avg_temp'].includes(field) && !Number.isFinite(inputData[field])) {
      console.error(`[YIELD SERVICE] Validation error: Invalid numeric field: ${field}`);
      throw new Error(`Invalid numeric field: ${field}`);
    }
    if (['Area', 'Item'].includes(field) && typeof inputData[field] !== 'string') {
      console.error(`[YIELD SERVICE] Validation error: Invalid string field: ${field}`);
...
//     mode: 'text',
//     pythonOptions: ['-u'],
//     pythonPath: process.env.PYTHON_PATH || 'C:\\Users\\Tauseef Ahmad Ghug\\AppData\\Local\\Programs\\Python\\Python313\\python.exe',
//     scriptPath,
//     args: [JSON.stringify(inputData)],
//     timeout: 10000
//   };
...
//       try {
//         if (code !== 0) {
//           console.error('[YIELD SERVICE] Python script exited with non-zero code:', code);
//           return reject(new Error(`Python script failed with exit code ${code}`));
//         }
//         if (!output.trim()) {
//           console.error('[YIELD SERVICE] No output received from Python script');
//           return reject(new Error('No output received from Python script'));
//         }
//         const result = JSON.parse(output.trim());
//         if (result.status === 'error') {
...
//           return reject(new Error(result.message));
//         }
//         if (!Number.isFinite(result.prediction) || !Number.isFinite(result.confidence)) {
//           console.error('[YIELD SERVICE] Invalid result format:', result);
//           return reject(new Error('Invalid result format: missing or invalid prediction or confidence'));
//         }
//         console.log('[YIELD SERVICE] Prediction successful:', result);
//         resolve(result);
//       } catch (parseErr) {
//         console.error(`[YIELD SERVICE] Parse error: ${parseErr.message}, raw output: ${output}`);
//         reject(new Error(`Failed to parse prediction result: ${parseErr.message}`));
//       }
//     });
